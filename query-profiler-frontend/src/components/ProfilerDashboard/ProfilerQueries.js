      ...
      <div className="queries-header">
        ...
        <div className="queries-controls">
          <button 
            ...
            onClick={() => {
              // Create a new state with all queries expanded
              const allExpanded = {};
              processedData.forEach(group => {
                allExpanded[group.name] = true;
              });
              setExpandedQueries(allExpanded);
            }}
          >
            Expand All
          </button>
          ...
          <button 
            ...
            onClick={() => {
              // Create a new state with all queries collapsed, except the currently selected one
              const allCollapsed = {};
              if (selectedProfile) {
                // Keep the group containing the selected profile expanded
                const groupWithSelected = processedData.find(group => 
                  group.queries.some(q => q.id === selectedProfile.id)
                );
                if (groupWithSelected) {
                  allCollapsed[groupWithSelected.name] = true;
                }
              }
              setExpandedQueries(allCollapsed);
            }}
          >
            Collapse All
          </button>
